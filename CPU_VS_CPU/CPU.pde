...
ArbreCP = new ArrayList<Arbre>() ;
 ...
 int f = 3;
 remplireArbre(WP,cope,ArbreCP,f,Score1 ,Score2,WP); 
 a.Ar = ArbreCP ; 
 ...
 if(WP==-1)  
  ...
  go(jeux.player2.pawns.get(ArbreCP.get(s.id).id) , ArbreCP.get(s.id).movement,jeux.player2,jeux);
    ...
    go(jeux.player1.pawns.get(ArbreCP.get(s.id).id) , ArbreCP.get(s.id).movement,jeux.player1,jeux);
...
{  
  ...
  { if(i==1) { return cope.player1 ;}
   ...
   return cope.player2;
       ...
       public void remplireArbre(int r, controle cope ,ArrayList<Arbre> ArbreCP,int f,int Score1 , int Score2,int WP)
      {  
        controle avant= copier(cope);
         ...
         {   
           for(int j =1 ;j<=18 ;j++)
           { 
             select(p.pawns.get(i) , j ,p,cope,ArbreCP,f,r,i ,Score1 ,Score2,WP);
             cope = copier(avant);
             p = replayer(r, cope);             
             size =p.pawns.size();            
   ...
   public void select(pawn pw ,int movement , player p, controle cope,ArrayList<Arbre> ArbreCP,int f,int r,int i ,int Score1 , int Score2,int WP)
   {    
        int x = pw.x ;
        int y = pw.y ;
        int z ,n,d;
        ...
        if(tester(pw ,x ,y)){Ajarbre( x , y , movement,cope,ArbreCP,f,r,i,0,Score1 ,Score2 ,pw.x ,pw.y,WP);}
        else{
        ...
        if(tester(pw ,x ,y)){ 
        Ajarbre( x , y , movement,cope,ArbreCP,f,r,i,4,Score1 ,Score2,pw.x ,pw.y,WP);}}
   ...
   {    int x = pw.x ;
     ...
     if(!tester(pw ,x ,y)){cope.move3(cope,p,(pw.y-3)*50+25 ,(pw.x-3)*50+25,z,n,d);}}
        ...
        public void Ajarbre(int x ,int y , int movement, controle cope,ArrayList<Arbre> ArbreCP,int f,int r,int i,int nb,int s1,int s2,int x1 , int y1,int WP)
        {  pawn p = new pawn(x,y);
           Arbre A = new Arbre(movement ,cope.player1.score ,cope.player2.score,p, nb ,s1,s2 , x1 , y1,r,WP);
           A.id= i ;
           ArbreCP.add(A); 
           if(f!=1){
              r=-r ; 
              remplireArbre(r,cope,A.Ar,f-1 ,s1,s2,WP);   
           }     
       }
